...
export const Sidebar: React.FC<SidebarProps> = ({
 ...
 >
 {/* Brand Header */}
 <div className="p-5 border-b border-black/[0.05] flex items-center gap-3">
 ...
 <div className="flex items-center gap-2 bg-black/[0.03] hover:bg-black/[0.05] transition px-3 py-2 rounded-xl border border-black/5 ">
 ...
 >
 <option value="" className="">เลือกสถานีต้นทาง...</option>
 {stations.map((st) => (
 <option key={`start-${st.id}`} value={st.id} className="">
 ...
 <div className="flex items-center gap-2 bg-black/[0.03] hover:bg-black/[0.05] transition px-3 py-2 rounded-xl border border-black/5 ">
 <Navigation className="w-4 h-4 text-mrt-b shrink-0" />
 <select
 value={endStation?.id || ''}
 onChange={(e) => {
 const s = stations.find((st) => st.id === e.target.value) || null;
 ...
 className="bg-transparent text-xs w-full font-semibold focus:outline-none appearance-none cursor-pointer"
 >
 <option value="" className="">เลือกสถานีปลายทาง...</option>
 {stations.map((st) => (
 <option key={`end-${st.id}`} value={st.id} className="">
 {st.nameTH} ({st.nameEN}) - {st.id}
 </option>
 ))}
 </select>
 ...
 {/* Grid Tabs */}
 <div className="grid grid-cols-5 gap-0.5 p-0.5 rounded-lg bg-black/[0.03] border border-black/5 ">
 ...
 : isSelectedEnd
 ? 'bg-mrt-b/5 border-mrt-b/40 text-mrt-b '
 : 'bg-black/[0.01] hover:bg-black/[0.03] border-black/5 '
 }`}
 >
