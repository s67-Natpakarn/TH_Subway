    ...
    >
      ...
      >
        ...
        <div className="p-5 border-b border-black/[0.05] dark:border-white/[0.05] flex items-center gap-3">
          ...
          <div>
            <h1 className="text-lg font-bold tracking-tight m-0 leading-none">ThaiTransit Minimal</h1>
            <span className="text-[10px] opacity-50 font-medium">ระบบคำนวณเส้นทางรถไฟฟ้า 8 สาย</span>
          </div>
        </div>

        {/* Station Selectors */}
        <div className="p-4 flex flex-col gap-3 bg-black/[0.01] dark:bg-white/[0.01] border-b border-black/[0.03] dark:border-white/[0.03]">
          {/* Start Station */}
          <div className="relative">
            <label className="text-[9px] font-bold tracking-wider uppercase opacity-45 block mb-1 px-1">
              สถานีต้นทาง
            </label>
            <div className="flex items-center gap-2 bg-black/[0.03] dark:bg-white/[0.05] hover:bg-black/[0.05] dark:hover:bg-white/[0.08] transition px-3 py-2 rounded-xl border border-black/5 dark:border-white/5">
              <MapPin className="w-4 h-4 text-bts-s shrink-0" />
              <select
          ...
          <div className="flex justify-center -my-2 relative z-10">
            <button
              onClick={handleSwap}
              className="p-1.5 rounded-full border border-black/5 dark:border-white/10 bg-white dark:bg-[#2c2c2e] hover:scale-105 active:scale-95 shadow-md transition text-gray-500 hover:text-black dark:text-gray-400 dark:hover:text-white"
            >
          ...
          <div className="relative">
            ...
            <div className="flex items-center gap-2 bg-black/[0.03] dark:bg-white/[0.05] hover:bg-black/[0.05] dark:hover:bg-white/[0.08] transition px-3 py-2 rounded-xl border border-black/5 dark:border-white/5">
              <Navigation className="w-4 h-4 text-mrt-b dark:text-blue-400 shrink-0" />
              <select
                value={endStation?.id || ''}
                onChange={(e) => {
                  const s = stations.find((st) => st.id === e.target.value) || null;
          ...
          {calculatedRoute ? (
            ...
            <div className="flex flex-col h-full">
              ...
              <div className="mb-4">
                ...
                {/* Search Input */}
                <div className="flex items-center gap-2 bg-black/[0.02] dark:bg-white/[0.03] px-3 py-2 rounded-lg border border-black/5 dark:border-white/5 mb-3">
                ...
                {/* Grid Tabs */}
                <div className="grid grid-cols-5 gap-0.5 p-0.5 rounded-lg bg-black/[0.03] dark:bg-white/[0.05] border border-black/5 dark:border-white/5">
                  {['all', 'bts', 'mrt', 'arl', 'srt'].map((tab) => (
                    <button
                      key={tab}
                      onClick={() => setActiveTab(tab as any)}
                      className={`py-1 text-[9px] font-bold rounded uppercase transition ${
                        activeTab === tab
              ...
              {/* Station Listing */}
              <div className="flex-1 overflow-y-auto flex flex-col gap-1 max-h-[260px] pr-1">
                    ...
                    return (
                      <div
                        ...
                        className={`p-2 rounded-xl border transition flex justify-between items-center ${
                          isSelectedStart
                            ? 'bg-bts-s/5 border-bts-s/40 text-bts-s'
                            : isSelectedEnd
                            ? 'bg-mrt-b/5 border-mrt-b/40 text-mrt-b dark:text-blue-400'
                      ...
                      >
                        <div className="flex flex-col">
                          <div className="flex items-center gap-1.5">
                            ...
                            <span className="text-[9px] opacity-50">({st.nameEN})</span>
                          </div>
                          ...
                          <div className="flex gap-0.5">
                            <button
                              ...
                              className={`px-1.5 py-0.5 text-[8.5px] font-bold rounded transition border ${
                                isSelectedStart
                                  ? 'bg-bts-s border-bts-s text-white'
                            ...
                            <button
                              ...
                              className={`px-1.5 py-0.5 text-[8.5px] font-bold rounded transition border ${
                                isSelectedEnd
                                  ? 'bg-mrt-b border-mrt-b text-white'
                                  : 'border-black/10 dark:border-white/10 hover:bg-black/5 dark:hover:bg-white/5'
                              }`}
                            >
                              ปลายทาง
      ...
      <button
        ...
        className="h-12 w-7.5 rounded-r-xl glass border-l-0 shadow-lg flex items-center justify-center cursor-pointer hover:bg-white dark:hover:bg-[#1c1c1e] text-gray-500 hover:text-black dark:text-gray-400 dark:hover:text-white transition-all duration-300"
      >
